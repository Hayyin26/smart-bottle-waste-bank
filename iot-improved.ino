...
// --- MODE OPERASI ---
// MODE 1: Default User (Simple) - Gunakan user_id yang sama untuk semua transaksi
// MODE 2: QR Login (Advanced) - User scan QR untuk login, setiap user punya akun sendiri
#define USE_QR_LOGIN false  // Set true untuk aktifkan QR login, false untuk default user

// Default user ID (digunakan jika USE_QR_LOGIN = false)
const char* default_user_id = "9db3ac82-dc1c-4f28-abe2-a8482986735f";

// API endpoint untuk QR login (hanya digunakan jika USE_QR_LOGIN = true)
// Ganti YOUR_IP dengan IP komputer yang running web server
const char* api_get_user = "http://192.168.1.100:3000/api/iot/get-user";

// Session variables (untuk QR login)
...
GateState gateState = WAIT_BOTTLE; // Default state
...
// --- FUNGSI GET USER ID DARI SESSION (untuk QR login) ---
...
void loop() {
  // Handle serial commands (untuk QR login)
  ...
  // Periodic session check (untuk QR login)
